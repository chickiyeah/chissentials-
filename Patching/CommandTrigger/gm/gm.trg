	...
	ELSEIF args.length == 2 && $isnumber:args[0]
		...
		IF list.contains(args[1])
			IF args[0] == "0"
				...
				print("관리자 "+$playername+"님이 "+args[1]+"&f 님의 게임모드를 서바이벌모드로 설정했습니다.")
				FOR i = 0:size
				nick = players.get(i)
			...
			ELSEIF args[0] == "1"
				...
				print("관리자 "+$playername+"님이 "+args[1]+"&f 님의 게임모드를 크리에이티브모드로 설정했습니다.")
				FOR i = 0:size
				nick = players.get(i)
				list.remove(nick.getName())
				ENDFOR
			...
			ELSEIF args[0] == "2"
				...
				print("관리자 "+$playername+"님이 "+args[1]+"&f 님의 게임모드를 모험모드로 설정했습니다.")
				FOR i = 0:size
				nick = players.get(i)
				list.remove(nick.getName())
				ENDFOR
			...
			ELSEIF args[0] == "3"
				...
				print("관리자 "+$playername+"님이 "+args[1]+"&f 님의 게임모드를 관전자모드로 설정했습니다.")
				FOR i = 0:size
				nick = players.get(i)
				list.remove(nick.getName())
				ENDFOR
