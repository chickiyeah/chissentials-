...
ELSE
ENDIF
IF $isop || $haspermission:"chissentials.tpa" || $haspermission:"chissentials.admin" || $haspermission:"chissentials.user"
IF args.length == 1
	...
	IF list.contains(args[0])
		IF $playername != args[0]
			...
			player($playername).sendMessage(color("&6&l치센셜 &f&l: &e&l"+args[0]+" &f님에게 tpa요청을 했습니다"))
			player(args[0]).sendMessage(color("&6&l치센셜 &f&l: &e&l"+$playername+" &f님으로부터 tpa요청이 들어왔습니다"))
			...
			#CMDOP "tpatimer "+args[0]+" "+$playername
		FOR i = 0:size
			nick = players.get(i)
		...
		ENDFOR
			configYml.save(configFile)
		ELSE
			player(args[0]).sendMessage(color("&6&l치센셜 &f&l: &4&l자기 자신에겐 tpa요청을 보낼수 없습니다"))
		ENDIF
	ELSE
