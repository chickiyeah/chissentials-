...
IF args.length == 1
	...
	ELSEIF args[0].equals("accept")
		...
		ELSE
			...
			redpane = item("RED_STAINED_GLASS_PANE", 1)
			...
			limepane = item("LIME_STAINED_GLASS_PANE", 1)
			limepanem = limepane.getItemMeta()
			limepanem.setDisplayName(color("&4&l준비 안됨"))
			limepane.setItemMeta(limepanem)
			
			glass = item("GLASS_PANE", 1)
			fence = item("IRON_BARS", 1)
			head1 = item("PLAYER_HEAD", 1)
			headmeta1 = head1.getItemMeta()
			profile1 = player.getHandle().getProfile()
			profileField1 = headmeta1.getClass().getDeclaredField("profile")
			profileField1.setAccessible(true)
			profileField1.set(headmeta1, profile1)
			headmeta1.setDisplayName($playerName)
			head1.setItemMeta(headmeta1)
			
			head2 = item("PLAYER_HEAD", 1)
			headmeta = head2.getItemMeta()
			profile = target.getHandle().getProfile()
			profileField = headmeta.getClass().getDeclaredField("profile")
			profileField.setAccessible(true)
			profileField.set(headmeta, profile)
			headmeta.setDisplayName(target.getName())
			head2.setItemMeta(headmeta)
